...
- Drop the removed example app from the `buildArgs` documentation
...
- Build child-table columns through the shared column rule, so a fieldset's children are no longer dropped
...
- Update fixtures/docs from ATextarea to ATextboxInput
...
- change return values of getAvailableTransitions
...
- allow custom component for link fields
...
- use the correct API response shape for fetching records
...
- accept doctype as a string in getRecord
...
- relegate building columns to component
...
- add fieldset support to resolveSchema
...
- add helper functions for casting doctype responses
...
- add new client option for setting up data clients in Stonecrop
...
- add utility functions to registry and composable
...
- add guard for value changes in HST
...
- update xstate dependency + add tests
...
- fix stonecrop documentation generation
...
- throw error if Stonecrop plugin isn't installed
