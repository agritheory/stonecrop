	...
	resolveSchema(doctype: Doctype, visited?: Set<string>): SchemaTypes[] {
		...
		for (const field of schemaArray) {
			...
			if ('fieldtype' in field && field.fieldtype === 'Link') {
				...
				if (!link) {
					// oxlint-disable typescript/no-unsafe-type-assertion -- SchemaTypes union narrowed to ValueField by fieldtype === 'Link' check; options may not exist on all members
					const linkDoctype =
