...
- a859205: Vue components now ship their real types — props, emits, slots and exposes — where every one of them previously resolved to `any`.
- a859205: Import the package's own stylesheet from its entry, so components are styled without a separate `./styles` import.
...
- Remove var() fallbacks that disagreed with the token floor
...
- Build the records list through the shared column rule, so links and child tables no longer stringify into a cell
...
- Stop fetching the record list and rendering a truncation banner; the records table now fetches its own pages via getRecords.
...
- Desktop now fetches list and record data through Stonecrop rather than waiting on the host; load-records and load-record remain as notifications for host-side side effects.
- Resolve unsaved-record identity through @stonecrop/stonecrop instead of restating the scheme, and stop emitting load-record for a draft: the record does not exist yet, so the fetch it asked for could only fail and every host had to recognise a private id format to suppress it
- Remove the recordIdField prop. Record identity is declared per doctype, and a per-shell string could not answer a per-doctype question.
- Hold a new record's data in the component rather than HST, and route New Record to /{doctype}/new. Fixes silent data loss: a draft's HST node never existed, so every field write threw and the typed data survived only in a cached object any addRecord invalidated. A draft is now also seeded with the doctype's declared defaults.
- BREAKING: remove the hardcoded row Delete affordance, the handleDelete method and the confirmFn prop. Desktop no longer invents a DELETE action; removal is a declared workflow outcome and confirmation belongs to the host
- Drop the doctype list's record count, which reported how many records HST held rather than how many exist, and note when a records list is only a page.

### Patches

- Re-export ActionEventPayload from @stonecrop/stonecrop, so the shell that emits it and the composable that consumes it can no longer drift apart.
- Correct the README's data-flow contract: Desktop reads through Stonecrop and the host-registered DataClient, so load-record and load-records are notifications rather than fetch requests, and drop two documented APIs that never existed.
...
- Build row links from the doctype's declared primaryKey instead of assuming record.id, falling back to id when none is declared. List rows of natural-key doctypes now navigate.
...
- fix ActionSet `dropdownOpen` ref typing (boolean[] instead of the inferred never[]), which surfaced once the fullstack example began type-checking
...
- update fetcher logic for table data
...
- add flatten schema fields function
...
- provide link navigator and resolver functions
...
- fetch record details directly on navigation
...
- remove table-specific responsibility
...
- correctly resolve schema from registry
...
- fix failing desktop tests + reorganize tests
...
- fix desktop click handler issue
...
- add emits and handlers to Desktop component
...
- update desktop story to use HST
...
- update table rows and columns format
...
- add command palette desktop component
...
## 0.2.53
Thu, 07 Nov 2024 13:02:28 GMT

_Version update only_

## 0.2.52
Thu, 07 Nov 2024 07:38:42 GMT

_Version update only_

## 0.2.51
Thu, 07 Nov 2024 07:32:46 GMT
...
## 0.2.42
Wed, 16 Oct 2024 07:47:14 GMT

_Version update only_

## 0.2.41
Wed, 16 Oct 2024 06:45:08 GMT
