...
Throwing rejects the action and no state is written. The client stores the result's `record`, the adapter's read of the record after the action, never this return; a return carrying the doctype's declared key names the record read, which is how a handler that creates one says so.
