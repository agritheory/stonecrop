...
- Drop the unused data-theme stylesheets from the published bundle and remove var() fallbacks that disagreed with the token floor
...
- Add ASegmentedControl and ExpandButton components, and honor a field's height in AForm's layout style
...
- Add `resolvedFieldsToColumns`, the shared rule for which resolved fields a table cell can render
...
- remove explicit theming on components
...
- create new resolved field types
...
- pass label prop to AFieldset + improve rows passthrough
...
- remove doctype-specific schemas
...
- remove auto-masking from components
...
- separate data layer from the schema
...
- handle infinite loop when setting up form component
...
- change prop from readonly -> readOnly
...
- Edited API.md to document new sorting and filtering features in ATable component.
...
- updated styling
-  Updated form labels to match table heading; consolidated similar styles.
