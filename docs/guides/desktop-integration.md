...
Nothing about this seam assumes GraphQL. `desktop/playground` ships a `RestDataClient` over plain `fetch`, and `nuxt/playground` ships one over a third-party API whose schema shares nothing with Stonecrop's.
...
What that does *not* do is drop the stale key or move the route, because both need the id you dispatched and it is inside `args`, which that layer must not parse. If you handle `@action` yourself, those two steps are yours. `desktop/playground/components/View.vue` is the worked non-Nuxt example.
