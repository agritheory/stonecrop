...
export default defineNuxtPlugin({
	...
	setup() {
		...
		// Register doctypes with the slug from the URL (filename-based)
		// The Desktop component uses currentDoctype (from URL) to look up in registry
		for (const [slug, doctypeConfig] of doctypeMap.entries()) {
			const doctypeInstance = Doctype.fromObject(doctypeConfig)

			// Register with both the URL slug and the computed slug for compatibility
			registry!.addDoctype(doctypeInstance)
			// Also add with URL slug key for Desktop lookup
