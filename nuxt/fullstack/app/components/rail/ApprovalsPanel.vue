<template>
	<div class="rail-panel">
		<p v-if="items.length === 0" class="rail-panel__empty">No pending approvals</p>
		<ul v-else class="rail-panel__list">
			<li v-for="item in items" :key="item.id">
				<button type="button" class="rail-panel__row" @click="open(item)">
					...
					<span class="rail-panel__row-meta">{{ item.role }}</span>
				</button>
			</li>
		</ul>
	</div>
...
const items = [
	{ id: 'appr-credit', name: 'Credit hold release', role: 'Credit Manager' },
	{ id: 'appr-ship', name: 'Ship-from override', role: 'Warehouse Lead' },
]
...
function open(item: (typeof items)[number]) {
	actionSet.present({
		id: item.id,
		view: StubPreview,
		props: { title: item.name, body: `Pending ${item.role} approval.` },
	})
}
</script>
