<template>
	<div class="rail-panel">
		<p v-if="items.length === 0" class="rail-panel__empty">No conversations</p>
...
const items = [
	{ id: 'col-address', name: 'Re: Delivery address change', from: 'customer@example.com' },
	{ id: 'col-warehouse', name: 'Ready to ship ORD-1042', from: 'warehouse@example.com' },
]
...
function open(item: (typeof items)[number]) {
	rail.present({
		id: item.id,
		view: StubPreview,
		props: { title: item.name, body: `Message from ${item.from}.` },
	})
}
</script>
